...
import {FetchPriceQuery, FetchPriceQueryInRange} from './price-query.actions';
...
export class PriceQueryFacade {
  ...
  fetchQuoteInRange(symbol: string, dateFrom: Date, dateTo: Date) {
    this.store.dispatch(new FetchPriceQueryInRange(symbol, dateFrom, dateTo));
  }
}
